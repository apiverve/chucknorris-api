...
class ChucknorrisValidationException implements Exception {
  final List<String> errors;

  ChucknorrisValidationException(this.errors);

  @override
  String toString() => 'ChucknorrisValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/chucknorris?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class ChucknorrisClient {
  ...
  ChucknorrisClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/chucknorris',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ChucknorrisValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ChucknorrisValidationException(errors);
    }
  }

  /// Execute a request to the Chuck Norris Jokes API.
  ///
  /// Parameters are validated before sending the request.
  Future<ChucknorrisResponse> execute() async {
    if (apiKey.isEmpty) {
      throw ChucknorrisException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ChucknorrisResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ChucknorrisException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ChucknorrisException('Resource not found');
      } else {
        throw ChucknorrisException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ChucknorrisException) rethrow;
      throw ChucknorrisException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Chuck Norris Jokes API client.
class ChucknorrisException implements Exception {
  final String message;

  ChucknorrisException(this.message);

  @override
  String toString() => 'ChucknorrisException: $message';
}
